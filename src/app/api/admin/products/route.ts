...
export async function POST(req: NextRequest) {
  ...
  const { name, short_description, description, regular_price, sale_price, status, stock_status, featured, category_ids, images, audio_urls } = body
  ...
  try {
    const product = await createProduct({
      ...
      audio_urls: Array.isArray(audio_urls) && audio_urls.length ? audio_urls : undefined,
    })
