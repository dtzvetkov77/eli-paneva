    ...
    <div className="pt-16">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        {programProducts.length > 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
            {staticPrograms.map(prog => (
              <div key={prog.title} className="border border-[var(--border)] p-8 flex flex-col">
                <div className="flex gap-3 mb-4">
                  <span className="text-xs border border-[var(--sage)] text-[var(--sage)] px-3 py-1">{prog.format}</span>
                  <span className="text-xs border border-[var(--border)] text-[var(--text-muted)] px-3 py-1">{prog.duration}</span>
                </div>
                <h3 className="font-serif text-xl text-[var(--text-dark)] mb-3">{prog.title}</h3>
                <p className="text-[var(--text-muted)] text-sm leading-relaxed flex-1 mb-6">{prog.desc}</p>
        ...
        <div className="border-t border-[var(--border)] pt-12">
          <h2 className="font-serif text-2xl text-[var(--text-dark)] mb-6">Свързани услуги</h2>
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <Link
                key={link.href}
                href={link.href}
                className="text-sm border border-[var(--border)] px-4 py-2 text-[var(--text-muted)] hover:border-[var(--sage)] hover:text-[var(--sage)] transition-colors"
              >
                {link.label}
              </Link>
            ))}
